...
function MyBody(props) {
  ...
  return (
    <ol>
      {/* 첫번째 경고창은 고정된 메세지를 알림창으로 띄워준다. props로 전달된
      기능을 자식 컴포넌트에서 그대로 사용하는 형식이다. 아래 링크를 클릭하는 경우 알림창을 닫으면 화면이 새로고침 된다. */}
      <li><a href='/' onClick={()=>{props.onMyAlert1();}}>프론트앤드</a></li>
      <ul>
        {liTag1}
      </ul>
      {/* 이벤트 객체를 통해 화면이 새로고침 되지 않도록 요청을 중단시킨다 React는 기본적으로 비동기방식으로 화면을 전환하므로 화면이 새로고침되면 안된다. 이럴 경우 초기화면으로 전환되기 때문이다. */}
  ...
  return (
    <div className='App'>
      ...
      /** 화살표 함수를 통해 경고창을 띄우는 기능을 전달 */
        ...
        /** 익명함수를 토앻 매개변수로 받은 메세지를 경고창으로 띄우는 
         기능을 전달 */
